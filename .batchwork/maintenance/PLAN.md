...
- [ ] #002 [Race Conditions] Move `initGlobals()` inside `PersistentPreRunE` on rootCmd so flags are parsed before global state reads them (`cmd/root.go`)
- [ ] #003 [Type Safety] Add nil-params guard or document contract that `Fetch()` always receives non-nil map (`alphavantage.go:54`)
...
- [ ] #019 [Reliability] Wire rate limiter into MCP server; enforce per-source rate limits for MCP tool calls (`mcp/server.go`)

## P2 — Medium

- [ ] #006 [Code Quality] Replace string-based `exitCode()` with sentinel error types (`ErrRateLimit`, `ErrAuthRequired`, `ErrInvalidInput`) and `errors.Is()` (`cmd/root.go`)
...
- [ ] #008 [API Contracts] Validate `action` against `Endpoints()` in Reddit `Fetch()`; validate `limit` range (clamp to 1-100); source default subreddit from config (`reddit.go`)
...
- [ ] #011 [Reliability] Check and log errors from `LoadUserSources()` and `LoadCatalog()` in `initGlobals()`; surface warnings unless `--quiet` (`cmd/root.go:152-155`)
